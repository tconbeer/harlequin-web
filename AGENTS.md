...
- **The vendored CLI reference is the one exception to "a page is a file"** — `<HsqlReference />` resolves to `static/artifacts/hsql-reference.md` rather than to anything in `src/docs`, so `/docs/headless/reference.md` carries the generated reference verbatim.
