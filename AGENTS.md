...
- **Clicking a result lands on the match.** A result links to `/docs/<slug>?q=<query>#<anchor>`, and `search_highlight.svelte` finds the query in the rendered page, paints it, and scrolls to the first match at or inside the section the link named.
- **It paints; it does not rewrite.** A docs page is a compiled mdsvex component whose text reaches `src/mdsvex/components/*.svelte` as slot content — renderable, not readable — so highlighting has to start from the DOM. (`h2.svelte` hit the same wall computing its own id and answers it the same way.) But it builds `Range`s over the nodes as they are and hands them to `CSS.highlights`, so nothing Svelte rendered is modified, cleanup is one `delete`, and a phrase can span element boundaries the way a `<mark>` around one text node could not. The `::highlight(search-hit)` rule lives in `src/app.css`, not in the component: Svelte scopes component styles with a generated class a highlight pseudo-element cannot carry, so the rule would match nothing and then be pruned. Where `CSS.highlights` is missing the page still scrolls and still counts.
- **The page scrolls itself, rather than the browser doing it.** Heading ids come from `slugify(slotValue)` with `bind:this`, so they do not exist when the browser processes the fragment — which is what `prerender.handleMissingId: "ignore"` is for. And `figure.svelte` renders an `<img>` with no dimensions on it, so every figure above the match moves it when it decodes; the scroll is re-aimed once after the layout settles.
- **`src/lib/search_highlight.ts`** is the part with no DOM in it — flattening text nodes into one string and mapping an offset back to the node it came from — so it has a test. The nodes are joined with a newline so the end of one paragraph and the start of the next cannot read as one word.

- **The tests are the guarantee.** `search.test.ts` runs the grammar and the ranking over four made-up pages, built with the same `buildIndex()` the real corpus uses. `search_highlight.test.ts` covers the offset mapping. `search_index.test.ts` runs over the real 71: that the dictionary is sorted (`termRange` binary-searches it, and out of order it returns wrong answers rather than failing), that postings point where they claim — for every section, a word only that section uses finds that section's page — and that the index stays inside its size budget. The budget is a tripwire: if the corpus grows into it, stop shipping section text and fetch it for the excerpt rather than raising the number.
