...
- **One corpus, four consumers** — `/docs/<slug>.md`, `/llms.txt`, `/llms-full.txt` and `/api/docs/v1*` all read `buildCorpus()`. The markdown a caller gets is the same bytes whichever one it asked; a second path that produces markdown would be a second sanitizer, and the first place they disagree is a page that says two different things.
