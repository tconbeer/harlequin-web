...
`ssh` was still running, so it was almost certainly waiting for input — a passphrase, a password, or a first-connection host key confirmation — that nobody typed. Run it once by hand to answer the prompt (`ssh -N db_prod`), add the key to your agent, and then add [`--ssh-batch-mode`](/docs/ssh/options) to anything unattended so the next failure is immediate and says which credential it wanted.
